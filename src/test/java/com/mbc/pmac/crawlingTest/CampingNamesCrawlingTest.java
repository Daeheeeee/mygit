...
class CampingNamesCrawlingTest {
	...
	void test() {
		...
		String campUrl = "https://gocamping.or.kr/bsite/camp/info/list.do?pageUnit=10&searchKrwd=&listOrdrTrget=c_rdcnt&searchLctCl=52&pageIndex=";
		
		// 캠핑장 이름들을 담아볼까
		List<String> campNameList = new ArrayList<>();
		
		for (int pageNum=1; pageNum <= 28; pageNum++) {
		
			try {
				
				// 1페이지 분량(접속)
				Document doc = Jsoup.connect(campUrl + pageNum).get();
